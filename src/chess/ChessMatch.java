...
public class ChessMatch {
	...
	public ChessPiece performChessMove(ChessPosition sourcePosition, ChessPosition targetPosition) {
		...
		if (movedPiece instanceof Pawn
			...
			enPassantVulnerable = null; // nenhuma peça esta vuneravel
		}
	...
	private boolean testCheck(Color color) {
		...
		for (Piece p : opponentPieces) {
			...
			if (mat[kingPosition.getRow()][kingPosition.getColunm()]) { // rei não esta em check
				return true;
			}
		}
		return false;
	...
	private boolean testCheckMate(Color color) {
		if (!testCheck(color)) { // se não estiver em check
			return false;
		}
		...
		for (Piece p : lista) {
			...
			for (int i = 0; i < board.getRows(); i++) {
				for (int j = 0; j < board.getColunms(); j++) {
					if (mat[i][j]) {// se eu estiver no I e no J que seja um movimento possivel
